...
public class ChatHub(ChatService chatService, UserService userService) : Microsoft.AspNetCore.SignalR.Hub
{
    public async Task SendMessage(string message, string? targetCustomerId = null)
    {
        if(string.IsNullOrEmpty(message)) throw new ArgumentNullException(nameof(message),"should not be null");
